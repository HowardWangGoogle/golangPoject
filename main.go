...
func main() {
	...
	// fmt.Print("REDISOPT", redisOpt.Addr)
...
func runTaskProcessor(config util.Config, redisOpt asynq.RedisClientOpt, store dbsq.Store) {
	...
	// fmt.Print("taskProcessor", taskProcessor.Start())
...
func runGrpcServer(config util.Config, store dbsq.Store, taskDistributor worker.TaskDistributor) {
...
func runGatewayServer(config util.Config, store dbsq.Store, taskDistributor worker.TaskDistributor) {
	server, err := gapi.NewServer(config, store, taskDistributor)
...
func runGinServer(config util.Config, store dbsq.Store) {
